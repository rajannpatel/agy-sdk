...
- A prompt that says `You are the <role> for <project-alias>` is a complete role
...
A Router must run `workshop run <project-alias> -- context`, classify the request,
...
Do not gate role preflight behind `workshop launch <project-alias> && ...`. Some
