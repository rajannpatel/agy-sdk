...
```bash
workshop run agy-sdk -- agent-role router
...
- For informational requests, answer directly and stop. Use narrow read-only
  ...
  brief for questions such as "Which GitHub Actions are used for SDKcraft?",
...
- For verification-only requests, do not produce an Architect brief. If the
  ...
  Otherwise, give the exact `workshop run agy-sdk -- ...` command or a
...
- For implementation-oriented requests, classify the repository area as one of:
  documentation, runtime, SDK packaging, test, dashboard, build tooling, or
  out-of-scope.
...
## Architect Brief Format
    ...
    ## Affected area
    <documentation | runtime | SDK packaging | test | dashboard | build tooling>
