...
- `workshop launch agy-sdk`
- `workshop refresh agy-sdk`
- `workshop run agy-sdk -- ...`
- `tools/workshop-shell`

When launch and role preflight are chained, use `tools/workshop-shell -c`
rather than a raw `workshop launch agy-sdk && workshop run ...` command.
The wrapper treats the known already-launched `workshop exists` response as
success for `workshop launch agy-sdk`, so required preflight commands are
not skipped.

Inside Workshop, use the named project actions from `workshop.yaml`, such as
`context`, `doctor`, `test`, `lint`, `build`, and `clean`.
...
| --- | --- | --- |
| Workshop terminal mode | The agent runs commands through a terminal thread, CLI, or TUI. | Launch the agent from `tools/workshop-shell` or `workshop run agy-sdk -- shell`. |
...
- `shellcheck`
- `yamllint`
- project-local shell test scripts
